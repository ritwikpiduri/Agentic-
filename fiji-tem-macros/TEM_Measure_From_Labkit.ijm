// =============================================================================
//  TEM_Measure_From_Labkit.ijm  = CODE 2 (Labkit version, no ilastik needed)
//  Segments each raw image with a Labkit classifier you trained + saved, then
//  measures organelles, nucleus shape, multinucleation and micronuclei.
//
//  BEFORE running: train a classifier in Labkit and save it (see the steps I
//  sent), then set CLASSIFIER below to that .classifier file's full path.
//
//  Labkit segmentation labels classes 0,1,2,... in the order you added them.
//  So CLASS_VALUES here are 0-based. Keep the order matching your Labkit labels.
// =============================================================================

// ---- point this at your saved Labkit classifier ----
var CLASSIFIER = "";   // e.g. "C:\\Users\\you\\Desktop\\organelles.classifier"

// ---- classes in the SAME order you added them in Labkit (0-based) ----
var CLASS_VALUES = newArray( 0,          1,        2,    3,             4,      5,        6);
var CLASS_NAMES  = newArray("Background","Nucleus","ER","Mitochondria","Golgi","Vacuole","LipidBody");

// ---- calibration: microns per pixel for each magnification ----
var CAL_MAG = newArray(2000,   2600,     11000,    22000);
var CAL_PX  = newArray(0.0072, 0.005454, 0.001355, 0.0006479);

// ---- settings ----
var NEAR_FAR_UM      = 1.0;
var MIN_OBJ_UM2      = 0.01;
var MIN_NUC_UM2      = 2.0;
var MICRONUC_MIN_UM2 = 0.05;
var PIXEL_SIZE_UM    = 0.005;
...
if (CLASSIFIER == "" || !File.exists(CLASSIFIER)) {
    CLASSIFIER = File.openDialog("Select your saved Labkit .classifier file");
}

setBatchMode(true);
inRoot = getDirectory("Choose the folder of RAW images to segment + measure");
outDir = getDirectory("Choose an OUTPUT folder for the result CSVs");
loadCalibration(findCalib(inRoot, outDir));

Dialog.create("Scale for this run");
Dialog.addChoice("Pixel size comes from:", newArray("Magnification in each filename", "One magnification for the whole run"), "Magnification in each filename");
...
total = countTiffs(inRoot);
idx = 0;
print("\\Clear");
print("Segmenting + measuring " + total + " images with Labkit...");
processTree(inRoot);
...
showMessage("Done", "Measured " + idx + "/" + total + " images.\n" +
    (fallbackCount > 0 ? ">> " + fallbackCount + " image(s) used the fallback scale.\n" : "") +
    "CSVs saved in:\n" + outDir);
...
function processTree(dir) {
    ...
    for (i = 0; i < list.length; i++) {
        ...
        else if (isTiff(list[i])) { idx = idx + 1; processImage(dir + list[i], dir, list[i]); }
    }
}
function processImage(path, dir, name) {
    ...
    orig = getTitle();
    px = calibratePx(name);                    // microns per pixel for this image

    // --- segment with the saved Labkit classifier ---
    run("Segment Image With Labkit", "input=[" + orig + "] segmenter_file=[" + CLASSIFIER + "] use_gpu=false");
    seg = getTitle();
    if (seg == orig) {                          // find the new segmentation window
        for (w = 1; w <= nImages; w++) { selectImage(w); if (getTitle() != orig) { seg = getTitle(); w = nImages + 1; } }
    }
    selectWindow(seg);
    if (bitDepth() == 24) run("8-bit");
...
function calibratePx(name) {
    pw = PIXEL_SIZE_UM;
    src = "fallback";
    if (FIXED_PX > 0) { pw = FIXED_PX; src = "fixed"; }
    else {
        mag = parseMag(name);
        if (mag > 0) { p = pxForMag(mag); if (p > 0) { pw = p; src = "mag"; } }
    }
